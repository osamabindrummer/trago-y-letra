...
export interface Author { id: string; slug: string; canonical_name: string; aliases: string[]; country: string; birth_year: number; death_year?: number; bio_es: string; featured_works: string[]; status: 'draft' | 'needs_review' | 'published' | 'rejected'; reviewed_at: string }
export interface Work { id: string; author_id: string; original_title: string; display_title_es: string; publication_year: number; language: string; identifiers: Record<string, string>; notes: string }
export interface Ingredient { name: string; amount: number; unit: string }
export interface Drink { id: string; name_es: string; aliases: string[]; category: string; alcoholic: boolean; ingredients: Ingredient[]; steps: string[]; glassware: string; garnish?: string; zero_proof_alternative_id?: string; recipe_note: string }
...
export interface Catalog { authors: Author[]; works: Work[]; drinks: Drink[]; recommendations: Recommendation[]; evidence: Evidence[]; sources: Source[] }
...
export interface PublicCatalog { generated_at: string; authors: PublicAuthor[]; drinks: Drink[]; sources: Source[] }
